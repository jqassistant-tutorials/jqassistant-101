...
- sets severity thresholds for printing warnings and breaking the build
...
The setup above activates the pre-defined group `spring-boot:Strict`.
...
There's also a pre-defined group `spring-boot:Default` containing less rules which might be sensible in existing projects.
