...
[.lead]
// tag::lead[]
Demonstrates definition and verification of allowed dependencies between packages.
// end::lead[]
...
The desired architecture of a Java project is usually described using terms like `Module`, `Component` or `Layer` and
allowed dependencies between them.
During implementation such elements are mapped to code structures.
One approach therefore is using packages, e.g. packages at a defined level are assigned the role of a `Component`.

This tutorial uses the following package structure:
...
The project consists of three components `a`, `b` and `c` with the following defined dependencies:

- `b` -> `a`
- `c` -> `a`

Any other dependency is not allowed.
The same restriction holds for circular dependencies between components, e.g. `a` -> `b` -> `a`.
...
NOTE: The concepts depends on the result of `maven:MainArtifact` which therefore is referenced within `requiresConcepts`.
...
Using the rules explained above it is already possible to avoid cycles if the defined dependencies are cycle-free.
But in larger projects with many components it may happen that such a problem is introduced by accident.
For this reason an additional constraint `structure:CyclicComponentDependencies` helps detecting such issues early:
