...
[.lead]
// tag::lead[]
This tutorial showns you how you can analyse Java's annotation
with jQAssistant which analyse Java annotations.
// end::lead[]
...
In our first example we will a Cypher query to find all classes
annotated with `@Foo` and a second query to find all values
for the `name` attributs of the annotation.

A first we will have a look at the source of the `@Foo`
annotation and it's usage in the class `Dings``
...
your project you have to look any type
with an annotation of the type `you.company.project.anno.Foo`.
The correspondending query in Cypher will look like
...
query operates on tree nodes and not two. So why three nodes?
...
play. This nodes represents the concrete annotation with
it's concrete values found in the code. You can image
...
it's value belongs to the annotation of the annotated
...
labled with `Value` and linked to the annotation representing
the concrete annotation with a relation labled with `HAS`.
...
and checks if it has a `HAS` relation to a node labled with `Value`.
