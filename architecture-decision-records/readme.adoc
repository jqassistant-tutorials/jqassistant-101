...
Stephan Pirnbaum <stephan.pirnbaum@buschmais.com> and Frode Meier
...
jQAssistant comes with the ability to execute rules (i.e. concepts and constraints) and to render the results inside AsciiDoc documents.
...
Therefore, the `jqassistant-maven-plugin` must be added to the `build/plugin` section in the `link:pom.xml[]` s shown in the following listing.
...
Secondly, the project uses the `jmolecules-ddd` dependency. This dependency provides helpful annotations which are used for some helpful rules predefined by jQAssistant. The following snippet is located in your `link:pom.xml[]` in the `dependencies` section:
...
<3> enables the `Default` group (The group is used here in <<Default-group-use, this section>>)
...
Further information how to use it can be found in the 101 about the plugin (TODO: Link).

In our example, after integrating the `jmolecules-ddd` dependency (as shown in <<pom-jmolecules, this section>>) and the `jqassistant-jmolecules-plugin` plugin (as shown in <<yaml, this section>>), we have to annotate all aggregate roots as `@AggregateRoot` and the entities as `@Entity`.
...
NOTE: The mentioned annotations are used exemplary used in `link:src/main/java/your/company/adr/project/Category.java[Category.java]` and `link:src/main/java/your/company/adr/project/ProductOption.java[ProductOption.java]`.
...
NOTE: The `Default` group is defined to be analyzed when jQAssistant is executed. This behaviour is explained in <<yaml, this section>>.
...
In the current state the execution of the maven build stops after analyzing the `adr:DeepLinking:NoDeepLinkingBetweenAggregates` constraint! To work around this you can change the severity of the constraint to `MINOR`. You can simply add `severity="minor"` to the constraint:
...
Firstly the `link:src/docs/asciidoc/index.adoc[]` is created (this is the default location for asciidoctor to look for adoc files):
...
<3> includes the content from `link:src/docs/asciidoc/adrs/001-Deep-Linking.adoc[001-Deep-Linking.adoc]` directly into this file.

Secondly the `link:src/docs/asciidoc/adrs/001-Deep-Linking.adoc[]` is created. It uses the basic structure of an Architecture Decision Record.
...
<6> This is an include statement that will be replaced by a report about the results of the jQAssistant analysis. For this the `jqassistant-asciidoctorj-extensions` as shown in <<Setting up Asciidoctor>> section is required.
