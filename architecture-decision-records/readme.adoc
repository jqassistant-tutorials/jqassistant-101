...
<4> tells jQAssistant to continue the execution if a rule with a high severity fails (explanation at <<continue_on_fail_explanation,the end of section "Defining Rules to Secure Decisions">>. The default case would be an execution stop.
...
[[securing]]
== Defining Rules to Secure Decisions
...
[[continue_on_fail_explanation]]
Whenever there will be a new violation, jQAssistant throws an error. As a result the build would normally fail until the issue is fixed. In this tutorial however the attribute `continue-on-failure` is set to `true` (See section <<yaml>>). That way jQAssistant continues the execution.

NOTE: Instead of continuing the build on a critical error it may also make sense to lower the severity of specific constraints to `MINOR`. That way, the decisions can be incorporated step-by-step.

TIP: It's also possible to change the default severity for constraints or tell jQAssistant to not throw an error on violation of a rule with a high severity. To learn more about this check the https://jqassistant.github.io/jqassistant/doc/2.0.0/manual/#_yaml_files[jQAssistant Manual]

[[documenting]]
== Documenting your ADR results

WARNING: For the documentation of your jQAssistant results it is important that your build finishes. With that in mind it may be necessary, to act according to <<continue_on_fail_explanation>> above.
...
.Processed html from index.adoc
image:index.png[]
...
TIP: To learn more about the include statement (6) check out the `generate-reports` tutorial in the link:https://github.com/jqassistant-tutorials/jqassistant-101[jqassistant-101] tutorials.

NOTE: For the full example, please check out the link:src/docs/asciidoc/adrs/001-Deep-Linking.adoc[001-Deep-Linking.adoc] directly.

This results in the following:

.Processed html from 001-Deep-Linking.adoc
image:001-Deep-Linking.png[]
