...
NOTE: This tutorial has been written for jQAssistant 2.0.3
...
- integrates the `jqassistant-maven-plugin` with version `2.0.3` into your Maven project.
...
Therefore, the directory contains the files `index.xml`, `package.xml` and `maven.xml`.


.jqassistant/index.xml
....
include::jqassistant/index.xml[]
....

<1> This is the root node of every xml document you want to define jQAssistant rules in
<2> This defines a group `Default` which just includes all groups matching the pattern `*:Default`
...
....
<1> This defines a matching group `package:Default` (matches the `\*:Default` pattern from the `Default` group) which itself includes all constraints matching `package:*`
...
<1> In a similar way `maven.xml` defines a group `maven:Default` but it includes all concepts matching `maven:*`
<2> The concept `maven:ProjectArtifactDependencies` defines the report type `plantuml-component-diagram`.
