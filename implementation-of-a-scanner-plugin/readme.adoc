...
1. <<Setup Maven Project>> (integrating necessary dependencies)
2. <<Define The Graph Model>> (defining the basic structure of the scanner with interfaces)
3. <<Implement The Scanner Plugin>> (implementing and embedding the plugin)
4. <<Testing>> (writing a basic test with the jQAssistant test API)
...
Dependencies for writing a scanner plugin for jQAssistant:
...
[source,xml]
.pom.xml dependencies for writing a scanner
----
include::pom.xml[tags=jQADependencies,indent=0]
----

Dependencies for testing a jQAssistant scanner plugin:

[source,xml]
.pom.xml dependencies for testing a scanner
----
include::pom.xml[tags=testDependencies,indent=0]
----

Dependency for reading csv files:
...
.pom.xml opencsv
----
include::pom.xml[tags=opencsv,indent=0]
----
...
[source,xml]
.resources//META-INF/jqassistant-plugin.xml
----
...
WARNING: This video was made for jQAssistant version 1.8, but most of the basic concepts remain the same.
...
3. link:http://extended-objects.org[eXtended Objects]