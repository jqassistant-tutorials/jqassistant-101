...
[.lead]
// tag::lead[]
This tutorial shows you how you can use jQAssistant in Maven project
by integrating the jQAssistant Plugin for Maven in a project.
// end::lead[]

NOTE: This tutorial is written for version 1.3.0 of jQAssistant. In
      further versions of jQAssistant some aspects might changed.


== Add the Plugin to the Maven POM

Open the top-level `pom.xml` of your project in any editor and
add a property called `jqassistant.version` to the properties
of your Maven project. It is a good pratice to managed version information
with the help of properties. So you can change a version only by
chaging a property.
...
Now add the plugin to the plugin managemenent section of
the `pom.xml`.
...
Now Maven knows that you would like to use the jQAssistant Plugin
for Maven and which version of it.

To run the plugin during the build you must at the plugin also
to plugin section.
...
With the shown configuration jQAssistant will be executed
during a normal Maven build.
...
...
[INFO]
[INFO] --- jqassistant-maven-plugin:1.3.0:scan (scan-and-analyze) @ parent ---
...
TIP: If you want skip the execution of jQAssistant for any reason,
     you can run Maven with `-Djqassistant.skip`.

jQAssistant will create during its run a new directory `jqassistant/store`
below the created `target` directory. It contains the Neo4j database with
the result of the jQAssistant's scan.

== Accessing the generated database

The Maven plugin also contains a goal called `server`. Running the plugin with this
goal runs an embedded Neo4j server and makes the Neo4j webconsole available.
On of the last lines of the output contains the server and port of the
webconsole.
